...
const getResult = (d: FoodEmoji) => d.result;
...
const Radar: React.FC<RadarProps> = ({
  ...
  const yScale = scaleLinear<number>({
    range: [0, radius],
    domain: [0, Math.max(...data.map(getResult))],
  });
  ...
  return parentWidth < 10 ? null : (
    <svg width={parentWidth} height={parentHeight}>
      <rect
        ...
        rx={14}
      />
      <Group top={parentWidth / 2 - margin.top} left={parentWidth / 2}>
